...
char *argv[];
{
  ...
  char extension[MAX_FILENAME_LEN]="gdt";
  ...
  *destination=0;
  ...
  if (io_error_count()==0) {
    string_for_all_sep(filelist,opt,","){
      ...
      if (*gdtout==0) {
         if (*destdir) {
            sprintf(gdtout,"%s/%s.%s",destdir,struct_name,extension);
         } else {
            sprintf(gdtout,"%s.%s",struct_name,extension);
         }
      }
      translate_gds_to_gdt(gdsin,(flag_stdout)?0:gdtout);
    } string_for_all_end;
    if (flag_stdin)  translate_gds_to_gdt(0,0);
    io_printf("##========================================\n");
  }
  io_summary();
  close_syslog_file();
